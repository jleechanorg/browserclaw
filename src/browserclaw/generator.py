...
def render_python_client(
    ...
    *,
    ...
    return f'''"""Generated by browserclaw from {catalog.source_har}.{" (DRY-RUN)" if dry_run else ""}"""{dry_run_marker}\n\nimport httpx\n{dry_run_guard}\n\n\nclass {class_name}:\n    def __init__(self, client: httpx.Client | None = None):\n        self.client = client or httpx.Client(follow_redirects=True)\n\n{methods_body}\n'''


def _render_dry_run_guard() -> str:
    """Emit a base class that fails loudly when tokens are missing in dry-run mode."""
    return '''
class MockSetTokenMissingError(RuntimeError):
    ...
    actual token values, e.g.::
        ...
        export MOCKSET_TOKENS=\'{"AUTHORIZATION_AUTHORIZATION": "Bearer xoxb-..."}\'
...
def require_mockset_token(placeholder: str) -> str:
    """Look up a token by placeholder name. Raises if missing."""
    tokens = _load_mockset_tokens()
    if placeholder not in tokens:
        raise MockSetTokenMissingError(
            f"Missing token for placeholder {placeholder!r}. "
    ...
    for i, entry in enumerate(entries, 1):
        ...
        for hdr in headers:
            ...
            if dry_run and name.lower() in _TOKEN_HEADERS:
                ...
                curl_parts.append(f"  -H \"${{{placeholder}:-}}\"")
                continue
            ...
            if dry_run and name.lower() == "cookie":
                ...
                for pair in value.split(";"):
                    ...
                    if cname.strip().lower() in _TOKEN_COOKIES:
                        ...
                        lines.append(f"_check_token {placeholder}")
                        rewritten_cookies.append(f"${{{placeholder}:-}}")
                    else:
                        rewritten_cookies.append(pair)
                cookie_value = "; ".join(rewritten_cookies)
                safe_value = cookie_value.replace("'", "'\\''")
                curl_parts.append(f"  -H 'Cookie: {safe_value}'")
                continue
