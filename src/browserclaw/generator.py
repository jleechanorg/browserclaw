...
### Via curl replay (verbatim HTTP replay)
```bash
# Replay all captured requests exactly as the browser sent them
bash curl_replay.sh

# Or replay a single request (edit curl_replay.sh and pick one block)
```

### Via browserclaw (re-capture)
```bash
# Extend the capture
...
# Full pipeline: capture + infer + generate + curl replay
browserclaw learn --url "{site_url}" --output-dir ./skill --save-skill
```
...
def _catalog_needs_tokens(catalog: EndpointCatalog) -> bool:
    token_headers = {"authorization", "x-api-key", "x-auth-token", "api-key", "api_key", "cookie"}
    for endpoint in catalog.endpoints:
        for hdr in endpoint.request_header_keys:
            if hdr.lower() in token_headers:
                return True
    return False
...
def render_python_client(
    catalog: EndpointCatalog,
    *,
    class_name: str = "BrowserClawClient",
    dry_run: bool = False,
) -> str:
    methods: list[str] = []
    dry_run_guard = _render_dry_run_guard() if dry_run else ""
    for endpoint in catalog.endpoints:
    ...
    methods_body = "\n\n".join(methods)
    docstring_suffix = " (DRY-RUN)" if dry_run else ""
    if dry_run:
        # Banner goes INSIDE the module docstring so the file is valid Python
        docstring = (
            f'"""Generated by browserclaw from {catalog.source_har}.{docstring_suffix}\n'
            "\n"
            "⚠️  DRY-RUN MODE: This client was generated from a mockset.\n"
            "    Tokens are NOT embedded. The class raises MockSetTokenMissingError\n"
            "    if MOCKSET_TOKENS env var (or ~/.config/browserclaw/mockset-tokens.json)\n"
            '    does not supply the required placeholders."""'
        )
    else:
        docstring = f'"""Generated by browserclaw from {catalog.source_har}.{docstring_suffix}"""'

    init_body = "self.client = client or httpx.Client(follow_redirects=True)"
    if dry_run and _catalog_needs_tokens(catalog):
        init_body += (
            "\n        tokens = load_mockset_tokens()\n"
            "        for k, v in tokens.items():\n"
            "            if k.startswith('COOKIE_'):\n"
            "                self.client.cookies[k[7:].lower()] = v\n"
            "            else:\n"
            "                parts = k.split('_')\n"
            "                hdr_name = '-'.join(p.capitalize() for p in parts)\n"
            "                self.client.headers[hdr_name] = v"
        )
...
def _render_dry_run_guard() -> str:
    """Emit a helper that loads MOCKSET_TOKENS at runtime. Raises if missing."""
    return '''
class MockSetTokenMissingError(RuntimeError):
    """Raised when a dry-run mockset client is called without the required tokens.

    Set MOCKSET_TOKENS env var to a JSON object mapping placeholder names to
    actual token values, e.g.::

        export MOCKSET_TOKENS=\'{"AUTHORIZATION": "Bearer xoxb-..."}\'

    Or write ~/.config/browserclaw/mockset-tokens.json with the same shape.
    Tokens are never read from .env files.
    """
    pass


def load_mockset_tokens() -> dict:
    """Load MOCKSET_TOKENS dict. Raises MockSetTokenMissingError if unset."""
    import json as _json
    import os as _os
    raw = _os.environ.get("MOCKSET_TOKENS")
    if raw:
        try:
            return _json.loads(raw)
        except _json.JSONDecodeError as e:
            raise MockSetTokenMissingError(
                f"MOCKSET_TOKENS env var is set but not valid JSON: {e}"
            ) from e
    cfg_path = _os.path.expanduser("~/.config/browserclaw/mockset-tokens.json")
    if _os.path.exists(cfg_path):
        with open(cfg_path) as _f:
            return _json.load(_f)
    raise MockSetTokenMissingError(
        "MOCKSET_TOKENS not set and ~/.config/browserclaw/mockset-tokens.json missing. "
        "Dry-run mockset clients require explicit token supply. Tokens are never "
        "read from .env files."
    )
'''
...
def render_curl_replay(har_path: str | Path, *, dry_run: bool = False) -> str:
    """Generate a shell script with verbatim curl commands for every HAR request.

    Skips binary/media resources. By default, auth tokens are included as-is so
    the script can be used to replay or as a starting point for reverse-
    engineering.

    When dry_run=True, Authorization headers and any Cookie containing a known
    token (d, session, token, auth, jwt, sid) are replaced with shell parameter
    expansions $MOCKSET_TOKENS_<PLACEHOLDER>. The script will fail loudly if
    those placeholders are not exported. Tokens are never embedded.
    """
    _SKIP_MIME = ("image/", "font/", "video/", "audio/", "text/css", "text/javascript")
    _SKIP_EXT = (".png", ".jpg", ".jpeg", ".gif", ".woff", ".woff2", ".ttf", ".ico", ".svg", ".css", ".js")
    _SKIP_HEADERS = frozenset((":method", ":path", ":scheme", ":authority", "content-length", "transfer-encoding"))
    _TOKEN_HEADERS = frozenset({"authorization", "x-api-key", "x-auth-token", "api-key", "api_key"})
    _TOKEN_COOKIES = ("d", "session", "token", "auth", "jwt", "sid")
    ...
    lines = [
        "#!/usr/bin/env bash",
        "# curl replay script — generated by browserclaw",
        f"# Source HAR: {har_path}",
        "# Replay every captured HTTP request verbatim.",
        "# IMPORTANT: Auth tokens/cookies may have expired — refresh before replaying.",
    ]
    if dry_run:
        lines += [
            "#",
            "# ⚠️  DRY-RUN MODE: tokens are NOT embedded. Provide them via",
            "#    export MOCKSET_TOKENS_AUTHORIZATION='Bearer xoxb-...'",
            "#    export MOCKSET_TOKENS_COOKIE_D='xoxc-...'",
            "#    before running this script. Missing placeholders will fail loud.",
            "#",
            "# Usage:",
            "#    export MOCKSET_TOKENS_AUTHORIZATION='Bearer <your-token>'",
            "#    export MOCKSET_TOKENS_COOKIE_D='<your-cookie-value>'",
            "#    bash curl_replay.sh",
            "",
            "set -euo pipefail",
            "",
            ...
            "  fi",
            "}",
            "trap '_check_tokens' EXIT",
            "_check_tokens() {",
            "  if (( ${#_missing[@]} > 0 )); then",
            "    echo \"ERROR: missing required MOCKSET_TOKENS_* vars:\" >&2",
            "    printf '  - %s\\n' \"${_missing[@]}\" >&2",
            "    exit 2",
            "  fi",
            "}",
        ...
        lines += [
            "#",
            "# Usage: bash curl_replay.sh",
            "#        bash curl_replay.sh 2>&1 | tee replay.log",
            "",
            "set -euo pipefail",
            "",
        ]
    ...
    for i, entry in enumerate(entries, 1):
        ...
        # Skip static assets
        if any(mime.startswith(s) for s in _SKIP_MIME):
            continue
        ...
        kept += 1
        lines.append("#" * 79)
        lines.append(f"# [{kept}] {method} {url}")
        lines.append(f"# HTTP {status} — {mime}")
        if resp_body:
        ...
        curl_parts = ["curl -sS", f"  -X {method}"]

        for hdr in headers:
            name = hdr.get("name", "")
            value = hdr.get("value", "")
            if not name or name.lower() in _SKIP_HEADERS or name.startswith(":"):
                continue

            if dry_run and name.lower() in _TOKEN_HEADERS:
                # Placeholder name matches what _detect_token_placeholders emits
                # (e.g. AUTHORIZATION, X_API_KEY, X_AUTH_TOKEN).
                placeholder = f"MOCKSET_TOKENS_{name.upper().replace('-', '_')}"
                lines.append(f"_check_token {placeholder}")
                # Double-quoted so ${VAR} expands. The value will be a literal
                # like "Bearer xoxb-..." so no shell escaping needed.
                curl_parts.append(f'  -H "{name}: ${{{placeholder}:-}}"')
                continue

            if dry_run and name.lower() == "cookie":
                # Replace known token cookies with MOCKSET_TOKENS_COOKIE_<NAME>.
                # The cookie name MUST be preserved so the server sees the
                # expected name=value structure. Use $VAR expansion with
                # double-quoted curl args (single-quotes block expansion).
                rewritten_cookies: list[str] = []
                for pair in value.split(";"):
                    pair = pair.strip()
                    if "=" not in pair:
                        rewritten_cookies.append(pair)
                        continue
                    cname, _cval = pair.split("=", 1)
                    if cname.strip().lower() in _TOKEN_COOKIES:
                        placeholder = f"MOCKSET_TOKENS_COOKIE_{cname.strip().upper()}"
                        lines.append(f"_check_token {placeholder}")
                        rewritten_cookies.append(
                            f"{cname.strip()}=${{{placeholder}:-}}"
                        )
                    else:
                        rewritten_cookies.append(pair)
                cookie_value = "; ".join(rewritten_cookies)
                # Double-quoted so ${VAR} expands.
                curl_parts.append(f'  -H "Cookie: {cookie_value}"')
                continue
    ...
    if kept == 0:
        lines.append("# No replayable requests found in HAR (only static assets).")
        lines.append("echo 'No replayable requests found.'")

    if dry_run:
        # Force trap to fire check
        lines += [
            "# Final token check (also runs via EXIT trap)",
            "_check_tokens",
        ]
...
def generate_bundle(
    ...
    *,
    ...
    client_path.write_text(render_python_client(catalog, dry_run=dry_run))
    ...
    # Always generate curl replay if a HAR is available
    resolved_har = Path(har_path) if har_path else (output / "capture.har")
    if resolved_har.exists():
        curl_path = output / "curl_replay.sh"
        if dry_run:
            # In dry-run mode, write a separate dry-run curl script that uses
            # $MOCKSET_TOKENS for any Authorization/Cookie values rather than
            # embedding the captured tokens directly.
            curl_path.write_text(render_curl_replay(resolved_har, dry_run=True))
        else:
            curl_path.write_text(render_curl_replay(resolved_har))
        curl_path.chmod(0o755)
        bundle["curl_replay"] = curl_path
