...
def render_python_client(catalog: EndpointCatalog, *, class_name: str = "BrowserClawClient") -> str:
    ...
    for endpoint in catalog.endpoints:
        ...
        query_arg_names = [_python_arg_name(key) for key in endpoint.query_keys]
        body_arg_names = [_python_arg_name(key) for key in endpoint.request_body_keys]
        ...
        query_payload = ", ".join([f'"{key}": {_python_arg_name(key)}' for key in endpoint.query_keys]) or ""
        json_payload = ", ".join([f'"{key}": {_python_arg_name(key)}' for key in endpoint.request_body_keys]) or ""
        url_for_format = _format_url(endpoint.url_template, path_param_names)
        # Form-encoded endpoints use data=, JSON endpoints use json=
